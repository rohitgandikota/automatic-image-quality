...
def detect_line_loss(noisy):
    ...
    for i in range(shape[0]):
        for j in range(shape[-1]):
            diff = noisy[i,:,j] - np.roll(noisy[i,:,j],-1)
            args = np.argwhere(diff == 0 )
...
def detect_block_loss_from_vinfo(vinfo,nbands):
    """
    TODO
    Write a code for detecting block losses given the parameters of deteceted line losses
    """
    block_loss = []
    for band in range(nbands):
        args = vinfo[vinfo[:,0]==band]
        unique = []
        for arg in args:
            if list(arg[-2:]) in unique:
                pass
            else:
                unique.append(list(arg[-2:]))
        for blob in unique:
            if blob[1] - blob[0] > 1:
                help_me = args[:,-2:]==blob
                help_me = np.logical_and(help_me[:,0],help_me[:,1])
                elems = args[help_me,:]
                cols  = elems[:,1]
                check = cols - np.roll(cols,-1)
                block_args = np.argwhere(check!=-1)
                for i in range(len(block_args)):
                    if i==0:
                        tuts = [elems[0,1], elems[block_args[i],1][0]]
                    else:
                        tuts = [elems[block_args[i-1][0]+1,1], elems[block_args[i][0],1]]
                    if tuts[1] - tuts[0] > 4:
                        block_loss.append([band, blob[0], blob[1], tuts[0], tuts[1]])    
    return block_loss

def detect_block_loss_from_hinfo(hinfo,nbands):
    """
    TODO
    Write a code for detecting block losses given the parameters of deteceted line losses
    """
    block_loss = []
    for band in range(nbands):
        args = hinfo[hinfo[:,0]==band]
        unique = []
        # get unique limits to draw boxes
        for arg in args:
            if list(arg[-2:]) in unique:
                pass
            else:
                unique.append(list(arg[-2:]))
        # for each limit draw the box 
        for blob in unique:
            if blob[1] - blob[0] > 1:
                help_me = args[:,-2:]==blob
                help_me = np.logical_and(help_me[:,0],help_me[:,1])
...
def visual_block_hinfo(block_info, image, inpath):
    output = np.zeros(image.shape)
    hello = block_info[:,1:]
    for hel in hello:
        output[hel[2]: hel[3], hel[0]:hel[1],0] = 255
    plt.imsave(inpath+'_Block_loss.png',output[:,:,0],cmap='gray')   
    
    
def visual_block_vinfo(block_info, image, inpath):
    """
    Visualize all block losses from both vinfo and hinfo
    """
    output = np.zeros(image.shape)
    hello = block_info[:,1:]
    for hel in hello:
        output[hel[0]: hel[1], hel[2]:hel[3],0] = 255
    plt.imsave(inpath+'_Block_loss.png',output[:,:,0],cmap='gray')  
...
def WriteQAMeta(scenequal, inpath):
    ...
    with open(designer_path+'/'+browse_id+'_SceneQual_META.txt','w') as fp:
        ...
        fp.writelines('BlockLossAreaPercentage='+str(scenequal["percentage_area_of_block_losses_in_pixels"])+'\n')
